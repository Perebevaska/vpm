...
const (
	...
	putQueueLen = 8
	// Малый порог: канал текст-приоритетный. Большой буфер под файлом = мелкий
	// ping-кадр стоит в FIFO за bulk-данными (HOL) → пинг «неопределён». 64КБ
	// держит HOL-задержку низкой ценой чуть меньшей скорости файла.
	maxOutBuf = 64 * 1024
